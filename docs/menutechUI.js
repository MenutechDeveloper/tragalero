...
(function () {
    ...
    class MenutechPlatformOrders extends HTMLElement {
        ...
        async loadAndRender() {
            ...
            let menuRecord = null;
            if (slug) {
                let res = await supabaseFetch('tragalero_menus', `slug=eq.${encodeURIComponent(slug)}`);
                if (!res || res.length === 0) {
                    res = await supabaseFetch('tragalero_menus', `slug=ilike.${encodeURIComponent(slug)}`);
                }
                if (res && res.length > 0) {
                    menuRecord = res[0];
                } else {
                    let fallback = await supabaseFetch('menutech_menus', `slug=eq.${encodeURIComponent(slug)}`);
                    if (!fallback || fallback.length === 0) {
                        fallback = await supabaseFetch('menutech_menus', `slug=ilike.${encodeURIComponent(slug)}`);
                    }
                    if (fallback && fallback.length > 0) menuRecord = fallback[0];
                }
            } else if (domain) {
                let res = await supabaseFetch('tragalero_menus', `domain=eq.${encodeURIComponent(domain)}`);
                if (!res || res.length === 0) {
                    res = await supabaseFetch('tragalero_menus', `domain=ilike.${encodeURIComponent(domain)}`);
                }
                if (res && res.length > 0) {
                    menuRecord = res[0];
                } else {
                    let fallback = await supabaseFetch('menutech_menus', `domain=eq.${encodeURIComponent(domain)}`);
                    if (!fallback || fallback.length === 0) {
                        fallback = await supabaseFetch('menutech_menus', `domain=ilike.${encodeURIComponent(domain)}`);
                    }
                    if (fallback && fallback.length > 0) menuRecord = fallback[0];
                }
            } else {
                // Default fallback: load first menu from tragalero_menus or menutech_menus
                const res = await supabaseFetch('tragalero_menus', 'order=created_at.desc&limit=1');
                if (res && res.length > 0) {
                    menuRecord = res[0];
                } else {
                    const fallback = await supabaseFetch('menutech_menus', 'order=created_at.desc&limit=1');
                    if (fallback && fallback.length > 0) menuRecord = fallback[0];
                }
            }
