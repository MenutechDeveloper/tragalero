...
async def verify():
    async with async_playwright() as p:
        ...
        try:
            ...
            await page.screenshot(path="final_adminCustomer_v6.png")
        except Exception as e:
            print(f"adminCustomer.html failed to show #app. Current URL: {page.url}")
            # Log all elements with .hidden to see why it's hidden
            hidden_elements = await page.evaluate("Array.from(document.querySelectorAll('.hidden')).map(el => el.id)")
            print(f"Hidden elements: {hidden_elements}")
            await page.screenshot(path="failed_adminCustomer_v6.png")

        print("Testing adminWeb.html...")
        # Since adminWeb.html redirects if mt_edit_owner_id is not set, we set it.
        await page.add_init_script("""
        ...
        try:
            ...
            await page.screenshot(path="final_adminWeb_v6.png")
            ...
            hidden_elements = await page.evaluate("Array.from(document.querySelectorAll('.hidden')).map(el => el.id)")
            print(f"Hidden elements: {hidden_elements}")
            await page.screenshot(path="failed_adminWeb_v6.png")
